...
- `SAVE_REQUEST` и `SAVE_RESULT` (удалены в более поздней версии)
- `CALC_PREVIEW` (заменён на CALC_RUN/CALC_INFO)

**Причина:** Протокол упрощён. Битрикс не нуждается в детальных уведомлениях о каждом действии пользователя.
...
2. **Реализация CALC_RUN**
   - Собирать полный снимок конфигурации
   - Формировать маппинг параметров на торговые предложения
   - Отправлять CALC_RUN при нажатии кнопки "Рассчитать"
   - Обрабатывать CALC_INFO с результатами

3. **Обработка закрытия**
   - Отправлять CLOSE_REQUEST с информацией о сохранении
   - Передавать данные конфигурации при закрытии
...
A: При нажатии кнопки "Рассчитать" → CALC_RUN, ответ → CALC_INFO.

**Q: Как передаются данные при закрытии?**  
A: Через CLOSE_REQUEST с параметром saved и данными конфигурации.
...
- [x] Реализована обработка INIT в App.tsx
- [x] Реализована отправка CALC_RUN
