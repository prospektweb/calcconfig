...
- Новые методы:
- `sendAddOfferRequest()` — отправить запрос на добавление ТП
- `sendRemoveOfferRequest(id, iblockId, iblockType, lang)` — отправить событие удаления оффера
...
- PostMessage события:
  ...
  - `btn-remove-offer` → `REMOVE_OFFER_REQUEST` с id оффера
...
4. **PostMessage события:**
  ...
  - Клик "btn-remove-offer" → отправляется REMOVE_OFFER_REQUEST с id
...
## [2026-01-13] Упрощение логики выбора деталей и унификация INIT

### Цель изменений
Упростить код путём удаления `SELECT_DETAILS_RESPONSE` и переход на модель, где Bitrix всегда отправляет полный `INIT` после любых изменений.

### Изменённые файлы

#### 1. `/src/lib/postmessage-bridge.ts`

**Удалено:**
- `SELECT_DETAILS_RESPONSE` из типа `MessageType`

**Изменено:**
- `sendSelectDetailsRequest()` теперь принимает параметр `binding: boolean`:
```typescript
sendSelectDetailsRequest(payload: { 
  binding: boolean,   // true = создать скрепление, false = заменить детали
  iblockId: number, 
  iblockType: string 
})
```
...
**Добавлено:**
- State `isSelectDetailDialogOpen` для управления диалогом выбора действия
- Функция `handleSelectDetailReplace()` - заменить деталь
- Функция `handleSelectDetailBinding()` - создать скрепление
- AlertDialog для выбора действия при наличии существующих деталей

**Изменено:**
- `handleSelectDetail()`:
  - Если деталей нет → сразу отправляет `SELECT_DETAILS_REQUEST` с `binding: false`
  - Если детали есть → показывает диалог выбора действия
  
- Обработчик `INIT` теперь **полностью заменяет** UI состояние:
  ```typescript
  // ЗАМЕНЯЕМ состояние (не добавляем!)
  setDetails(transformedDetails)
  setBindings(transformedBindings)
  ```
  - Если `preset` отсутствует → очищает `details` и `bindings`

- Обработчик `REFRESH_RESULT` также обновлён для полной замены состояния

### Поведение SELECT_DETAILS_REQUEST

#### Направление
iframe → Bitrix

#### Назначение
Запрос на открытие диалога выбора существующих деталей
...
#### Ожидаемое действие Bitrix
1. Открыть диалог выбора деталей
2. После выбора отправить полный `INIT` с обновлёнными данными
3. Если `binding: true` — создать группу скрепления с выбранными деталями
4. Если `binding: false` — заменить текущие детали на выбранные

### UI Изменения

#### Новый диалог: "Выбор детали"
Появляется при клике "Выбрать деталь", когда уже есть детали:
- Кнопка "Отмена" - закрывает диалог
- Кнопка "Заменить" - заменяет текущие детали (`binding: false`)
- Кнопка "Создать скрепление" - создаёт группу скрепления (`binding: true`)

### Принцип работы INIT

После любых изменений (добавление, удаление, выбор деталей) Bitrix отправляет полный `INIT`:
- Фрейм получает `INIT`
- Фрейм **полностью перестраивает** UI из `preset` и `elementsStore`
- Старое состояние деталей/скреплений **заменяется** новым
- Если `preset` пустой → UI очищается

Это упрощает логику и исключает необходимость в специфичных RESPONSE-типах для каждой операции.
