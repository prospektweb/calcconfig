...
import { CaretDown, CaretUp, Tag } from '@phosphor-icons/react'
...
export function PricePanel({ messages, isExpanded, onToggle }: PricePanelProps) {
  return (
        ...
        {isExpanded && (
          <ScrollArea className="h-[150px] px-4 pb-2">
            <div className="space-y-1">
              {messages.length === 0 ? (
                <p className="text-sm text-muted-foreground py-2">Нет данных по отпускным ценам</p>
              ) : (
                messages.map(msg => (
