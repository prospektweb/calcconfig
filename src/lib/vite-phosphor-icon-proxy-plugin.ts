...
/**
 * Vite plugin that proxies missing Phosphor icon imports to a fallback icon.
 * This prevents build errors when icons are referenced but don't exist in the package.
 * 
 * Originally from @github/spark/vitePhosphorIconProxyPlugin
 * Copied locally to remove dependency on @github/spark
 */
    ...
    transform(code, id) {
      ...
      while ((match = importRegex.exec(code)) !== null) {
        ...
        for (let i = 0; i < imports.length; i++) {
          ...
          const parts = importItem.split(/\s+as\s+/)
          ...
          if (!existingExportsCache.has(iconName)) {
            console.log(
              `[icon-proxy] Proxying non-existent icon: ${iconName} -> ${fallbackIcon}`
            )
        ...
        if (
          needsQuestion &&
          !imports.some(
            (imp) => imp === fallbackIcon || imp.startsWith(`${fallbackIcon} `)
          )
        ) {
