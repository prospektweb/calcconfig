# Калькулятор себестоимости для 1С-Битрикс

Веб-приложение для расчёта себестоимости и отпускных цен полиграфической продукции, встраиваемое в админ-панель 1С-Битрикс.

## Описание

Калькулятор работает **только внутри iframe** админ-панели Битрикс. Позволяет администраторам:
- Создавать конфигурации продуктов из деталей, материалов, операций и оборудования
- Рассчитывать габариты, вес, себестоимость и отпускные цены
- Сохранять результаты для применения к торговым предложениям

**Важно:** Все изменения локальны до нажатия кнопки «Сохранить».

## Быстрый старт

```bash
npm install
npm run dev
```

Для сборки продакшн-версии:
```bash
npm run build
```

Результат размещается на сервере по пути: `/local/apps/prospektweb.calc/`

## Документация

### 📖 Основная документация

- **[README_INTEGRATION.md](./README_INTEGRATION.md)** — краткий обзор интеграции
- **[BITRIX_INTEGRATION_CHANGELOG.md](./BITRIX_INTEGRATION_CHANGELOG.md)** — история изменений интеграции с Bitrix
- **[IMPLEMENTATION_SUMMARY.md](./IMPLEMENTATION_SUMMARY.md)** — детали реализации
- **[PRD.md](./PRD.md)** — требования к продукту
- **test-integration.html** — страница для локального тестирования

## Архитектура интеграции

### Режимы работы

**NEW_CONFIG** — создание новой конфигурации  
Активируется когда:
- У выбранных ТП нет конфигурации
- У ТП разные конфигурации

**EXISTING_CONFIG** — редактирование существующей  
Активируется когда:
- У всех выбранных ТП одинаковая конфигурация

### Протокол postMessage (v1.0.0)

Жизненный цикл взаимодействия:

```
iframe → Битрикс: READY (калькулятор загружен)
Битрикс → iframe: INIT (передача контекста и данных)
iframe → Битрикс: INIT_DONE (готов к работе)
...работа с конфигурацией...
iframe → Битрикс: CALC_PREVIEW (результат расчёта)
...пользователь нажимает "Сохранить"...
iframe → Битрикс: SAVE_REQUEST (запрос на сохранение)
Битрикс → iframe: RESPONSE (результат сохранения)
```

### Пример базовой интеграции

```javascript
const iframe = document.getElementById('calc-iframe');

window.addEventListener('message', (event) => {
    const msg = event.data;
    if (msg.source !== 'prospektweb.calc') return;
    
    switch (msg.type) {
        case 'READY':
            // Отправить INIT с данными
            iframe.contentWindow.postMessage({
                source: 'bitrix',
                target: 'prospektweb.calc',
                type: 'INIT',
                payload: {
                    mode: 'NEW_CONFIG',
                    context: { siteId, userId, lang: 'ru' },
                    iblocks: { materials, operations, ... },
                    selectedOffers: [...]
                }
            }, '*');
            break;
            
        case 'SAVE_REQUEST':
            // Сохранить конфигурацию и обновить ТП
            await saveConfiguration(msg.payload);
            break;
    }
});
```

Подробности в [README_INTEGRATION.md](./README_INTEGRATION.md) и [BITRIX_INTEGRATION_CHANGELOG.md](./BITRIX_INTEGRATION_CHANGELOG.md)

## Структура проекта

```
/src/
├── components/
│   ├── calculator/           # Компоненты калькулятора
│   └── ui/                   # UI компоненты (shadcn)
├── hooks/
│   ├── use-config-kv.ts      # Hook для хранилища конфигурации
│   ├── use-postmessage.ts    # Hook для postMessage
│   ├── use-custom-drag.ts    # Drag & drop
│   └── use-mobile.ts         # Mobile detection
├── lib/
│   ├── postmessage-bridge.ts # Ядро postMessage API
│   ├── types.ts              # TypeScript типы
│   ├── bitrix-utils.ts       # Утилиты для Bitrix
│   ├── bitrix-transformers.ts
│   ├── bitrix-to-ui-transformer.ts
│   └── utils.ts
├── services/
│   └── configStore.ts        # Хранилище конфигурации
├── stores/                   # Дополнительные хранилища
├── styles/                   # Стили
├── types/                    # Типы TypeScript
└── App.tsx                   # Главный компонент
```

## Ключевые возможности

✅ Конструктор конфигураций из деталей и скреплений  
✅ Drag & Drop для изменения порядка элементов  
✅ Многоуровневые справочники материалов/операций/оборудования  
✅ Расчёт габаритов, веса, себестоимости, отпускных цен  
✅ Настройки округления и наценок с "красивыми" ценами  
✅ Стабильные pwcode для всех элементов UI  
✅ Полная интеграция с 1С-Битрикс через postMessage

## Технологии

- React 19 + TypeScript
- Tailwind CSS + shadcn/ui
- Vite
- Phosphor Icons
- Sonner (уведомления)

## Тестирование

### Локально

```bash
npm run dev
# Откройте test-integration.html в браузере
```

### С Битрикс

1. Соберите проект: `npm run build`
2. Разместите файлы на сервере в `/local/apps/prospektweb.calc/`
3. Создайте страницу с iframe в админке
4. Настройте обработчики postMessage

## Поддержка

При возникновении вопросов:
1. Изучите [README_INTEGRATION.md](./README_INTEGRATION.md) и [BITRIX_INTEGRATION_CHANGELOG.md](./BITRIX_INTEGRATION_CHANGELOG.md)
2. Посмотрите примеры в документации
3. Используйте test-integration.html для отладки

## Лицензия

MIT License - Copyright GitHub, Inc.
