...
```json
{
  ...
  "requestId": "req-123",
  "payload": {
    "binding": false,        // true = создать скрепление, false = заменить детали
    "iblockId": 100,
    "iblockType": "calculator_catalog"
  },
  "timestamp": 1234567890
...
**Назначение:** Запрос на открытие диалога выбора существующих деталей

**Параметры:**
- `binding: boolean` - если `true`, создать группу скрепления из выбранных деталей; если `false`, заменить текущие детали
- `iblockId: number` - ID инфоблока деталей
- `iblockType: string` - тип инфоблока

**Ожидаемое действие Bitrix:**
1. Открыть диалог выбора деталей
2. После выбора отправить полный `INIT` с обновлёнными данными (`preset` и `elementsStore`)
3. Фрейм полностью перестроит UI на основе нового `INIT`
