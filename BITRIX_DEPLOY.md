...
- ✅ Обработка расчётов через `CALC_RUN`/`CALC_INFO`
...
**Причина:** Bitrix не обрабатывает сообщение `CALC_RUN`.

**Решение:**
1. Убедитесь, что Bitrix слушает postMessage события
2. Проверьте формат `CALC_RUN` в консоли
3. Реализуйте обработчик `CALC_RUN` на стороне Bitrix
4. Отправляйте `CALC_INFO` с результатами
